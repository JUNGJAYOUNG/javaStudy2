...
class ServerThread extends Thread{
	...
	//연결된 클라이언트와 데이터를 주고받을 스트림을 생성한다.
	InputStream is;
	OutputStream os;
	...
	public void sendAll(byte []data) throws Exception{
		//모든 클라이언트를 담아둔 list로부터 데이터를 하나씩 끄집어 내어오면
		//그것은 ServerThread 자료형이다.
		for(ServerThread t:ChatServer.list) {
			
			//각각의 ServerThread에는 연결한 클라이언트에게 데이터를 출력을 위한 스트림이 있다.
			//그 스트림을 통하여 데이터를 출력한다.
			t.os.write(data);
		}
	}	
...
public class ChatServer {
	...
	//ArrayList에 담는 자료형은 통신을 요청한 한명의 클라이언트와 계속하여 대화를 주고받는 일을
	//담당하는 쓰레드인 ServerThread를 담는다.
	...
	public static void main(String[] args) {
		...
		try {
			...
			while(true) {	//무한대기 상태로 클라이언트의 접속 기다림	
				...
				//연결된 클라이언트와 대화를 계속 주고받기위한 쓰레드 객체를 생성하고 쓰레드를 가동한다.
				ServerThread thread = new ServerThread(socket);
				thread.start();
				
				//접속한 모든 클라이언트와 통신을 담당하는 객체를 담아 두기 위한 객체를 
				list.add(thread);
			}
		}catch(Exception e) {
